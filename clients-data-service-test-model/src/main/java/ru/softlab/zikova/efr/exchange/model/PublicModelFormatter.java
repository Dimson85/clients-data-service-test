...
public final class PublicModelFormatter {
    ...
    /**
     * Представить модель (Данные о клиенте) в виде строки
     *
    ...
    public static String format(Client model) {
        ...
        if (model == null) {
            addNullModel(sb, "Данные о клиенте");
            return sb.toString();
        }
        addModel(sb, "Данные о клиенте");
        addField(sb, "Внутренний идентификатор клиента", toIndentedString(format(model.getId())));
        ...
        addField(sb, "Актуальность", toIndentedString(format(model.isIsActive())));
        return sb.toString();
    }

    /**
     * Представить модель (Данные о продукте) в виде строки
     *
     * @param model модель {@link Product}
     * @return строка
     */
    public static String format(Product model) {
        StringBuilder sb = new StringBuilder();
        if (model == null) {
            addNullModel(sb, "Данные о продукте");
            return sb.toString();
        }
        addModel(sb, "Данные о продукте");
        addField(sb, "Внутренний идентификатор продукта", toIndentedString(format(model.getId())));
        addField(sb, "Номер продукта", toIndentedString(format(model.getNumber())));
        addField(sb, "Тип продукта", toIndentedString(format(model.getType())));
        addField(sb, "ID клиента", toIndentedString(format(model.getClientId())));
        return sb.toString();
    }
