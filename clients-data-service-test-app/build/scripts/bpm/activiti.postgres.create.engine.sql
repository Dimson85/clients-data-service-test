...
--скрипты Softlab
--0) изменения по итогам нагрузки 1 релиза
...
create INDEX EFR_IDX_EVENT_SUBSCR_PROC_INST_ID on ACT_RU_EVENT_SUBSCR(PROC_INST_ID_);
create INDEX EFR_IDX_TASK_PARENT_TASK_ID on ACT_RU_TASK(PARENT_TASK_ID_ );

--1) Для пользователей с полными правами недостаточно места в переменных
